...
int main()
{
	...
	if (choice == 1)
	{
		...
		for (int i = 0; i < pcount; i++)
		{
			printf("|  %s\t\t  %d\t\t  %d\t\t  %d\t\t  %d\t\t  %d\t\t|\n", plist[i], at[i], bt[i], wt[i], tt[i], rt[i]);
		}
		...
		printf("|  \t\t\t\t\t\t  %f\t  %f\t  %f\t|\n", (float)(wsum/pcount), (float)(tsum/pcount), (float)(rsum/pcount));
	...
	else if (choice == 2)
	{
		...
		while (allover(bt2, pcount) != 1)
		{
			// decide most prior of all arrived processes
			minp = -1;
			for (int i = 0; i < pcount; i++)
			{
				if (minp == -1)
				...
				else if (at[i] <= clk && bt2[i] > 0 && priority[i] < priority[minp])
				{
					minp = i;
				}
			}
