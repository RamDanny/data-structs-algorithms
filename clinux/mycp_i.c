...
int main(int argc, char *argv[])
{
	...
	int targetfd = open(argv[2], O_RDWR);
	if (targetfd == -1)
	{
		targetfd = open(argv[2], O_RDWR | O_CREAT, S_IRWXU);
		if (targetfd == -1)
		{
	...
	{
		char iswrite = 'y';
		printf("Do you want to overwrite target file?");
		scanf(" %c", &iswrite);
		if (iswrite != 'y' && iswrite != 'Y')
		{
			printf("Copy stopped!\n");
			return 0;
		}
